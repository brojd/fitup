...
let stylesheetComponentTask = require('./tasks/stylesheet-component');
let autoprefixerTask = require('./tasks/autoprefixer');
let autoprefixerComponentTask = require('./tasks/autoprefixer-component');
...
gulp.task('client-stylesheet-component', stylesheetComponentTask(false));
gulp.task('client-stylesheet-dist', stylesheetTask(true));
gulp.task('client-autoprefixer', autoprefixerTask(false));
gulp.task('client-autoprefixer-component', autoprefixerComponentTask(false));
...
gulp.task('client-stylesheet-component-watch', function() {
  gulp.watch(['client/app/**/*.scss'], ['client-stylesheet-component']);
});

gulp.task('autoprefixer-watch', function() {
  gulp.watch(['client/boot.css', 'client/**/*.css'], ['client-autoprefixer', 'client-autoprefixer-component']);
});
...
gulp.task('serve', function(done) {
  runSequence(
    'clean',
    ['client-build', 'client-copy', 'client-stylesheet', 'client-stylesheet-component', 'livereload',
      'client-stylesheet-watch', 'autoprefixer-watch', 'client-stylesheet-component-watch'], 'server-start',
    done
  )
});
